...
- **Keybind:** Go to Game Settings -> Key Bindings -> FiveM and assign a key to the **Open Main Page** option registered by the `txadmin` command.
...
- **Teleport:** To waypoint, coordinates, copy current coordinates to your system clipboard, or other players
...
These ConVars control the behavior of the in-game menu, and are configured through the fxPanel Settings page. ConVars managed by the settings page should not be set manually.
...
These ConVars are set directly in your `server.cfg`. Use `setr` when a value must be replicated to clients; use `set` for server-only values.
...
**`/tpm`**
Teleport to the waypoint set on your map. Requires `players.teleport`.

**`/goto <id>`**
Teleport to a player by their server ID. Requires `players.teleport`.

**`/ban <target>`**, **`/kick <target>`**, **`/warn <target>`**
Open the menu focused on the matching player action. These commands require `players.ban`, `players.kick`, or `players.warn` respectively.

**`/announce <message>`**
Broadcast an announcement. Requires `announcement`.

**`/ticket`** or **`/report`**
Open the in-game ticket UI when reports are enabled.

**`/txAdmin-reauth`**
Retrigger the menu authentication process while the menu resource is enabled. No admin permission is required to run the retry.
