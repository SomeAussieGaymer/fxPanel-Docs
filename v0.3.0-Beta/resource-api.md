...
> All exports are called from server-side Lua via `exports['monitor']:exportName(...)`. Moderation action exports require an authenticated fxPanel admin as the source and enforce the appropriate permission check. Successful moderation actions are bridged back into fxPanel and logged in action history.
...
Persist custom player tags through fxPanel. These exports do not require the source player to be an admin; they emit structured trace messages that core processes into the player database.

### `addPlayerTag(serverId, tagId)`

Add a configured custom tag to a player.

```lua
exports['monitor']:addPlayerTag(targetPlayerId, 'vip')
```

### `removePlayerTag(serverId, tagId)`

Remove a configured custom tag from a player.

```lua
exports['monitor']:removePlayerTag(targetPlayerId, 'vip')
```
...
manage_tickets       -- Delete any ticket and override ticket auto-deletion
