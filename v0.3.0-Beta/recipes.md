...
YAML-based deployment automation for setting up server data folders. Recipes are jailed to the target folder and cannot write outside it. At the end of deployment, the target folder is checked for a `server.cfg` and a `resources` folder.
...
| `$requiresGithubToken` | Whether the deployer should prompt for a GitHub token |

---

## Context Variables

These variables are automatically populated during deployment and can be expanded with `{{variableName}}` syntax. `replace_string` expands variables in replacement text by default, and the deployer also runs a final `replace_string` pass with `mode: all_vars` over `server.cfg` after all recipe tasks finish.
...
| `{{addPrincipalsMaster}}` | `add_principal` lines for the master admin's linked identifiers, or a commented example if none are linked |
...
| `write_file` | Write content to a file. Optional `append` mode. |
...
Use `replace_string` with `mode: all_vars` when you want to expand every `{{variableName}}` placeholder inside an existing file. The `write_file` task writes its `data` exactly as provided.
