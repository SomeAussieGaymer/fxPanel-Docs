...
Build addons that extend fxPanel with custom routes, panel pages, widgets, Discord bot commands/events, and more.

> Addon server code runs in the addon runtime selected by fxPanel: a forked Node process where available, a worker fallback, or the in-process runtime used by default on Linux embedded hosts. Treat approved addons as trusted server-side code.
...
- **Discord bot modules** — Slash commands and Discord event handlers loaded by the standalone bot runtime
...
│       ├── discord-bot/
│       │   ├── commands/    ← Optional standalone bot commands
│       │   └── events/      ← Optional standalone bot events
│       ├── panel/
...
```json
{
  ...
  "id": "my-addon",              // Lowercase a-z, 0-9, hyphens. 3-64 chars.
  ...
  // — Standalone Discord bot entry points (optional) —
  "discordBot": {
...
Addon-owned Discord commands and events are first-class in v0.3.X. A running addon can ship slash commands and event listeners alongside its server routes and panel UI without modifying fxPanel's built-in `bot/` files.

If your addon wants to extend the standalone bot, add a `discordBot` section to `addon.json`:
...
1. Add a `discordBot` block to `addon.json`.
2. Create `discord-bot/commands/` and/or `discord-bot/events/` inside your addon.
3. Export command or event objects from those files.
4. Keep privileged logic in addon server routes and call them from Discord through `addonRoute(...)`.
5. Reload the addon, or use **Diagnostics -> Discord Bot -> Reload addons** if you only changed addon bot modules.

Rules:

- `commands` and `events` are both optional individually, but at least one must be present if `discordBot` exists.
- `rateLimit` is optional. When present, it becomes the default rate limit for that addon's slash commands, autocomplete handlers, buttons, modals, and select menus. Limits are applied per addon, per handler, and per Discord user.
- These paths are resolved relative to the addon root. Absolute paths and any `..` segment that would escape the addon directory are rejected during manifest validation.
- The standalone bot only loads Discord modules from addons that are currently in the `running` state.
- Addon command and event files are loaded through dynamic `import()`, so ESM default exports work even though the standalone bot runtime itself is CommonJS.

### Command Contract

Addon Discord command files can now export more than `data` and `execute`:
...
export default {
  ...
  buttons: {
    async myAction(interaction, bridge, context) {},
  },

  modals: {
    async saveForm(interaction, bridge, context) {},
  },

  stringSelectMenus: {
    async pickOne(interaction, bridge, context) {},
  },
};
```
...
- `autocomplete()` is called for slash-command option autocompletes.
- `buttons`, `modals`, and select-menu maps are keyed by the action name used in the custom ID helper.
- Component handlers receive a third `context` argument containing `addonId`, `kind`, `action`, `state`, `rawState`, `commandName`, and `filePath`.
- Individual component handlers can override the command-level rate limit by exporting `{ execute, rateLimit }` instead of a bare function.

### SDK Ergonomics

`createAddonDiscordSdk({ addonId, bridge })` now exposes two helpers for common interaction work:

- `discord.respondWithChoices(interaction, choices)` normalizes and replies to autocomplete interactions.
- `discord.interactions` creates addon-namespaced custom IDs that the standalone bot can route back into your addon.
...
const discord = createAddonDiscordSdk({ addonId: 'my-addon', bridge });

await discord.respondWithChoices(interaction, ['One', 'Two', 'Three']);

const buttonRow = new ActionRowBuilder().addComponents(
  discord.interactions.button(new ButtonBuilder(), 'openForm', {
    label: 'Open form',
...
const modal = discord.interactions.modal(new ModalBuilder(), 'submitForm', {
  title: 'Save player note',
  state: { playerId: 42 },
  components: [
...
The generated custom IDs follow a reserved addon namespace. Do not hand-roll IDs that overlap with built-in `fxpanel:*` IDs.

### Discord SDK Helpers

Addon Discord commands and events still receive the raw `bridge` helper at runtime, but the supported addon-facing API lives in `addon-sdk/discord`.

- `createAddonDiscordSdk({ addonId, bridge })` wraps the raw bridge with addon-aware helpers.
- `discord.request(type, payload)` and `discord.send(typeOrMessage, payload)` expose the underlying bridge when you need lower-level access.
- `discord.addonRoute({ method, path, body, interaction })` proxies into your addon's server routes without rebuilding the requester payload manually.
- `discord.addonRoute(...)` can also take explicit `requesterId`, `requesterName`, and `memberRoles` values when you are inside a non-interaction event.
- `discord.getRequesterPayload(interaction)` extracts the Discord user ID, display name, and member role IDs in the shape the bridge expects.
- `discord.getConfigSnapshot()`, `discord.resolveMemberRoles(uid)`, `discord.resolveMemberProfile(uid)`, `discord.refreshMemberCache()`, and `discord.reloadCommands()` expose the common standalone bot bridge requests with typed helpers.
- `createMockDiscordBridge(...)` lets you exercise command and event logic locally without a live bot runtime.
- `validateAddonDiscordManifest(...)` and `parseAddonDiscordManifest(...)` let you preflight a manifest or `discordBot` block in local tooling before you restart fxPanel.

### Command Example

When your addon uses `"type": "module"`, export Discord bot commands with a default export:

```js
// discord-bot/commands/hello.js
import { createAddonDiscordSdk } from 'addon-sdk/discord';
...
export default {
  data: new SlashCommandBuilder()
    .setName('addon-hello')
    .setDescription('Example command provided by an addon'),
  async execute(interaction, bridge) {
    const discord = createAddonDiscordSdk({ addonId: 'hello-world', bridge });
    const response = await discord.addonRoute({
      method: 'POST',
      path: '/discord/hello',
      body: { channelId: interaction.channelId },
      interaction,
    });

    if (response?.status !== 200) {
      const errorMessage = response?.body?.error ?? 'Addon route failed';
      await interaction.reply({ content: errorMessage, flags: MessageFlags.Ephemeral });
      return;
    }

    await interaction.reply({ content: response.body.message, flags: MessageFlags.Ephemeral });
  },
};
```
...
Event listeners follow the same idea: export a default object with `name`, optional `once`, and `execute`. Your `execute(...)` function receives the normal Discord.js event arguments first, followed by the bot client and the fxPanel bridge.

```js
// discord-bot/events/member-join.js
import { createAddonDiscordSdk } from 'addon-sdk/discord';

export default {
  name: 'guildMemberAdd',
  async execute(member, botClient, bridge) {
    const discord = createAddonDiscordSdk({ addonId: 'hello-world', bridge });

    await discord.addonRoute({
      method: 'POST',
      ...
      requesterName: member.user.tag,
      memberRoles: [...member.roles.cache.keys()].filter((roleId) => roleId !== member.guild.id),
    });
  },
};
```

Use the explicit requester fields in event handlers because there is no slash-command interaction object for `getRequesterPayload(...)` to read from.

For local iteration, `addon-sdk/discord` also exports `createMockDiscordBridge(...)`, which you can hand to `createAddonDiscordSdk(...)` to exercise command logic without a live bot connection.
...
If you want to validate a Discord manifest block before booting fxPanel, you can also do that directly from the SDK:

```js
import { validateAddonDiscordManifest } from 'addon-sdk/discord';

const result = validateAddonDiscordManifest({
  id: 'hello-world',
  discordBot: {
    commands: 'discord-bot/commands',
  },
...
- Addon Discord commands and events are only loaded while the addon is in the `running` state.
- If you changed addon command/event files, reloading the addon is enough. You can also use **Diagnostics -> Discord Bot -> Reload addons** to make the standalone bot reload addon command and event modules.
- If you changed Discord settings, embeds, or presence data, use **Diagnostics -> Discord Bot -> Resync runtime** instead.
- Any addon command/event load failures are surfaced in the **Diagnostics -> Discord Bot** page so you can see which addon file failed and why.

### Pairing Discord Commands With Addon Server Logic

The `addonRoute` bridge request forwards a request into your addon's existing server routes. That keeps privileged logic in the addon server process instead of duplicating it inside the Discord bot.
...
addon.registerRoute('POST', '/discord/hello', async (req) => {
  return {
    status: 200,
    body: {
      message: `Hello from ${req.admin.name}`,
    },
  };
});
...
`req.admin.permissions` in that route reflects the resolved fxPanel permissions for the Discord user. That includes both manually assigned admin permissions and any permission presets synced onto the linked admin account from Discord role mappings, so a real fxPanel admin account and a mapped Discord role can both be enforced through the same route logic.
...
Every addon's server entry imports from `addon-sdk` (shipped with fxPanel). The SDK is automatically resolved via `NODE_PATH` — no `npm install` needed. Addon-owned Discord command and event files can additionally import from `addon-sdk/discord`.
...
**How routing works:** When a request hits `GET /addons/my-addon/api/players/abc123/notes`, fxPanel authenticates the request, extracts the addon ID and remaining path, forwards everything to the addon's runtime channel, the SDK matches the route, and the return value becomes the HTTP response.

Authenticated addon API routes receive sanitized headers and a sanitized JSON request body only when the request `Content-Type` is `application/json`. Bodies larger than 1 MB are rejected before the addon handler runs.
...
> **HTTPS note:** The dedicated public addon server is currently HTTPS-only. If you configure a dedicated public port, fxPanel must also receive `TXHOST_ADDON_PUBLIC_TLS_KEY` + `TXHOST_ADDON_PUBLIC_TLS_CERT` or `TXHOST_ADDON_PUBLIC_TLS_KEY_FILE` + `TXHOST_ADDON_PUBLIC_TLS_CERT_FILE` in the host environment.
...
| Static text files | Serve CSS, JS, SVG, JSON, or HTML by reading files from your addon directory |

Public route responses are sanitized before leaving fxPanel. Return strings or JSON-compatible values; binary file responses are not a reliable public route transport in the current implementation.
...
| `body`    | `null`   | Dedicated public route requests currently do not forward a parsed body |
...
| ------ | ----------- | ----------- |
| Dedicated port | `https://host:<port>/` | When a public port is configured, routes are served directly by the HTTPS public addon server |
...
3. **Disabled** — If neither is set, the dedicated public server is not started
...
- **Input validation** — Treat all public route input as untrusted. Validate and sanitize `req.params` and `req.headers`
...
Addons request permissions in their manifest. The current manifest schema accepts only the permissions listed below. Approval of an addon still means trusting its server-side code; these permissions are enforced at specific fxPanel IPC boundaries rather than acting as a full sandbox.
...
| `players.read`      | Reserved for player data / player event access      |
| `players.write`     | Add or remove configured custom player tags         |
| `ws.push`           | Push real-time data to panel clients via WebSocket  |

The source currently enforces `storage`, `players.write`, and `ws.push` on the matching IPC operations. Player join/drop and moderation events are broadcast to running addon processes by core.
...
6. On the next boot or hot-reload cycle, the addon starts with the granted permissions
...
| `approved` | Permissions granted, will start on the next boot or hot-reload cycle |
...
2. When a change is detected in a server file, the addon's current runtime is gracefully stopped when possible
3. A new addon runtime is started with the updated code
...
├── server/
│   └── index.js        ← Server entry (runs in the addon runtime)
├── panel/
...
Your addon's server code usually runs in a forked Node process on hosts where a safe Node runtime is available. On Linux embedded hosts, fxPanel defaults to an in-process runtime because worker termination can crash the host isolate. The forked/worker runtimes receive a minimal environment:

- `ADDON_ID`
- `ADDON_RUNTIME`
- `NODE_ENV`
- `NODE_PATH`
- selected host process variables needed for execution and locale, including `PATH`, `HOME`, `LANG`, `LC_ALL`, `LC_CTYPE`, `TZ`, and `TERM` when present

Do not rely on the runtime as a hard security sandbox. Addon API requests strip sensitive admin headers before forwarding, and addon storage is scoped by core, but approved addon code should be treated as trusted host code.
...
- Server entries must import `from 'addon-sdk'`; addon-owned Discord command/event files should import `from 'addon-sdk/discord'` instead of a relative path
...
- Verify the port number is valid (1-65535). Ports below 1024 may require elevated privileges on some hosts.
- If fxPanel logs a TLS credential error, set `TXHOST_ADDON_PUBLIC_TLS_KEY` + `TXHOST_ADDON_PUBLIC_TLS_CERT` or the `_FILE` variants for the host process
- Check the console for `[addon:<id>]` bind errors
- If no port is configured, fxPanel does not start the dedicated public addon server. The main panel origin intentionally does not serve public addon routes.
