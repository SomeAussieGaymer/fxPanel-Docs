...
> **Important**
>
> The `serverId` parameter in action exports refers to the **admin** performing the action, not the target. The admin must be authenticated with fxPanel and have the required permission, otherwise the call will silently fail with a server-console error.
...
players.message      -- Direct message players
players.whitelist    -- Manage player whitelist
players.troll        -- Use troll actions on players

-- Player Actions (in-game menu)
players.heal         -- Heal players
players.spectate     -- Spectate players
players.teleport     -- Teleport to/bring players
players.freeze       -- Freeze players
players.playermode   -- Change player mode (godmode, noclip)
players.vehicleaccess -- Spawn/manage vehicles

-- Server Management
announcement         -- Send announcements
manage.admins        -- Manage admin accounts
console.view         -- View the live console
console.write        -- Execute commands in the console
...
-- All Permissions
all_permissions      -- Grants every permission
```
