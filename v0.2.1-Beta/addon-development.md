...
```json
{
  ...
  "dependencies": {
    "other-addon": ">=1.0.0"     // Addon ID → semver range
  },
  ...
  // — Settings (optional) —
  "settings": {
    "component": "MySettingsComponent" // Named export from panel entry
  },
...
> **Note:** The SDK's route matcher only supports simple `:param` segment matching (one segment per parameter). Regex-based patterns like `:path(.*)` are not supported. For SPA catch-all routing, register separate routes for `/` and `/:page`.
...
Add a `dependencies` object to your `addon.json`:
...
Keys are addon IDs, values are semver ranges.

### Behavior

- **Boot order** — Dependent addons start after their dependencies are running
...
- **Version mismatch** — If the dependency exists but its version doesn't satisfy the range, the addon fails with a version error
- **Circular dependencies** — Detected at boot and reported as an error; neither addon starts
...
Add a `customPermissions` array to your `addon.json`:
...
Add a `settings` object to your `addon.json`:
